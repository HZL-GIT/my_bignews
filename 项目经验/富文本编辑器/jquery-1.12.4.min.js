...
// 使用jq设置全局配置
// 为了使得这个全局配置在每一个页面的ajax请求都能使用，所以将其放入jq文件中，那么只要页面调用了jq.js的文件包，这个全局配置就会生效
$.ajaxSetup({
    // 所有的ajax发送之前会执行这个函数（一般在这设置请求头）
    ...
    error: function (xhr, status, error) {
        console.log(status);
        console.log(error);
        // 未获取token，用户未登录时，报Forbidden，跳转回登录页
        if (error == 'Forbidden') {
            alert('请先登录');
            window.location.href = './login.html';
            // 登录网址出错时，报Not Found，跳转404页面
        } else if (error == 'Not Found') {
            alert('发生404错误');
            window.location.href = '../项目经验/token/404界面.html'
        }
    }
});
